...
struct WhyUseBudgeView: View {
    ...
    private func select(_ name: String) {
        ...
        Task.detached(priority: .utility) {
            do { try await onboarding.updateUserProfile(uid: uid, fields: ["usingReason": name]) } catch {}
        }
    }
}
...
private extension WhyUseBudgeView {
    func promptOverlay(containerSize: CGSize) -> some View {
        ...
        return VStack(spacing: 10) {
            speechBubble(text: "How will you use Budge?", availableWidth: containerSize.width)
